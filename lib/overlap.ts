...
export interface AvailabilityBand extends Interval {
  ...
  awayIndices: number[];
  /**
   * Has class that day, but this band is before their own first class, or
   * after the last class anyone has. Free in the sense that nothing is booked,
   * but not free in the sense that matters here: they aren't on campus yet, or
   * campus has emptied for the day, so meeting means a trip made specially.
   */
 ...
 *
 * Only the gaps count. The morning before someone's first class and the evening
 * after the teaching day ends are not availability, they're a commute you'd be
 * asking for. Left in, they dominate the grid — every day is bright from 8am
 * and again from 5pm, which says nothing, while the hour-long gap at noon that
 * people could actually make gets no more weight than a Tuesday nobody is on
 * campus at all.
 *
 * The two ends are cut differently, because they aren't the same question.
 * Arriving is personal: nobody comes in early for a morning with nothing in it,
 * so a member starts counting at their own first class. Leaving is not: someone
 * whose last class ended at 1:20 is still standing on campus at 1:30, and
 * staying an hour is a smaller favour than travelling in — so they keep
 * counting until the last class of the day is out, whoever it belongs to.
 * Cutting the evening per member as well collapses the grid: personal windows
 * rarely intersect, and with a group of seven nothing rose above one free.
 *
...
export function availabilityBands({
  ...
  for (const day of days) {
    ...
    for (let i = 0; i < edges.length - 1; i++) {
      ...
      dayBusy.forEach((busy, mi) => {
        const arrives = arrival[mi];
        if (arrives === null) return; // not on campus at all today
        // Hasn't come in yet, or the day is over for the whole group.
        if (to <= arrives || (lastOut !== null && from >= lastOut)) {
          segOutside.push(mi);
          return;
        }
